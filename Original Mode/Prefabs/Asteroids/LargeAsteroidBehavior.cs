...
public class LargeAsteroidBehavior : MonoBehaviour
{
    ...
    {
        if (other.CompareTag("Player")) // Check if the collision is with the player.
        {
            // Handle player collision here.
            PlayerHealth playerHealth = other.GetComponent<PlayerHealth>();
            if (playerHealth != null)
            {
                // Inflict damage on the player or implement your player death logic here.
                playerHealth.TakeDamage(1); // You can adjust the damage amount as needed.
            }
        }
        else if (other.CompareTag("PlayerBullet") || other.CompareTag("EnemyBullet"))
        {
            DestroyAsteroid();
        }
    }
