...
public class PlayerHealth : MonoBehaviour
{
    ...
    private void Update()
    {
        if (Input.GetKeyDown(shieldActivationKey))
        {
            // Activate shield.
            ActivateShield();
        }
    ...
    private IEnumerator Respawn()
    {
        ...
        // You can set the respawn position based on your camera's position.
        Camera mainCamera = Camera.main;
        if (mainCamera != null)
        {
            transform.position = new Vector3(1920f, 1080f, 5f);
        }
        else
        {
            // If Camera.main is null, set a default respawn position (0, 0, 0).
    ...
    private IEnumerator RespawnEffect()
    {
        ...
        float endTime = startTime + blinkDuration;
